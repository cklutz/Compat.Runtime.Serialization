...
namespace Compat.Xml
{
    internal static class XmlExceptionHelper
    {
        ...
        {
            string s = SRHelper.Format(res, arg1, arg2, arg3);
            if (reader is IXmlLineInfo lineInfo && lineInfo.HasLineInfo())
            {
                s += " " + SRHelper.Format(SR.XmlLineInfo, lineInfo.LineNumber, lineInfo.LinePosition);
            }
        ...
        private static string GetWhatWasFound(XmlDictionaryReader reader)
        {
            ...
            switch (reader.NodeType)
            {
                case XmlNodeType.Element:
                    return SRHelper.Format(SR.XmlFoundElement, GetName(reader.Prefix, reader.LocalName), reader.NamespaceURI);
                case XmlNodeType.EndElement:
                    return SRHelper.Format(SR.XmlFoundEndElement, GetName(reader.Prefix, reader.LocalName), reader.NamespaceURI);
                case XmlNodeType.Text:
                case XmlNodeType.Whitespace:
                case XmlNodeType.SignificantWhitespace:
                    return SRHelper.Format(SR.XmlFoundText, reader.Value);
                case XmlNodeType.Comment:
                    return SRHelper.Format(SR.XmlFoundComment, reader.Value);
                case XmlNodeType.CDATA:
                    return SRHelper.Format(SR.XmlFoundCData, reader.Value);
            }
            return SRHelper.Format(SR.XmlFoundNodeType, reader.NodeType);
        }
        ...
        {
            return new XmlException(SRHelper.Format(SR.XmlInvalidConversion, value, type), exception);
        }
        ...
        {
            return new XmlException(SRHelper.Format(SR.XmlInvalidUTF8Bytes, value), exception);
        }
    }
}
