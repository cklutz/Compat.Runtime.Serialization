    ...
    internal sealed class CollectionDataContract : DataContract
    {
                ...
                if (helper.XmlFormatGetOnlyCollectionReaderDelegate == null)
                {
                    lock (this)
                    {
                        if (helper.XmlFormatGetOnlyCollectionReaderDelegate == null)
                        {
                            ...
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GetOnlyCollectionMustHaveAddMethod, DataContract.GetClrTypeFullName(UnderlyingType))));
                            }
                            if (IsReadOnlyContract)
                            {
                                ThrowInvalidDataContractException(helper.DeserializationExceptionMessage, null /*type*/);
        ...
        private class CollectionDataContractCriticalHelper : DataContract.DataContractCriticalHelper
        {
                ...
                if (itemType != null)
                {
                    ...
                    if (collectionContractAttribute != null)
                    {
                        if (collectionContractAttribute.IsItemNameSetExplicitly)
                        {
                            ...
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidCollectionContractItemName, DataContract.GetClrTypeFullName(UnderlyingType))));
                            }
                        ...
                        if (collectionContractAttribute.IsKeyNameSetExplicitly)
                        {
                            ...
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidCollectionContractKeyName, DataContract.GetClrTypeFullName(UnderlyingType))));
                            }

                            if (!isDictionary)
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidCollectionContractKeyNoDictionary, DataContract.GetClrTypeFullName(UnderlyingType), collectionContractAttribute.KeyName)));
                            }
                        ...
                        if (collectionContractAttribute.IsValueNameSetExplicitly)
                        {
                            ...
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidCollectionContractValueName, DataContract.GetClrTypeFullName(UnderlyingType))));
                            }

                            if (!isDictionary)
                            {
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidCollectionContractValueNoDictionary, DataContract.GetClrTypeFullName(UnderlyingType), collectionContractAttribute.ValueName)));
                            }
                ...
                : base(type)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.CollectionMustHaveGetEnumeratorMethod, DataContract.GetClrTypeFullName(type))));
                }

                if (itemType == null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.CollectionMustHaveItemType, DataContract.GetClrTypeFullName(type))));
                }
                ...
                : this(type, kind, itemType, getEnumeratorMethod, null, (string)null)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.CollectionMustHaveAddMethod, DataContract.GetClrTypeFullName(type))));
                }
                        ...
                        if (IsDictionary)
                        {
                            if (string.CompareOrdinal(KeyName, ValueName) == 0)
                            {
                                DataContract.ThrowInvalidDataContractException(
                                    SRHelper.Format(SR.DupKeyValueName, DataContract.GetClrTypeFullName(UnderlyingType), KeyName),
                                    UnderlyingType);
                            }
        ...
        private static bool HandleIfInvalidCollection(Type type, bool tryCreate, bool hasCollectionDataContract, bool createContractWithException, string message, string param, ref DataContract dataContract)
        {
            if (hasCollectionDataContract)
            {
                if (tryCreate)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(GetInvalidCollectionMessage(message, SRHelper.Format(SR.InvalidCollectionDataContract, DataContract.GetClrTypeFullName(type)), param)));
                }
            ...
            if (createContractWithException)
            {
                if (tryCreate)
                {
                    dataContract = new CollectionDataContract(type, GetInvalidCollectionMessage(message, SRHelper.Format(SR.InvalidCollectionType, DataContract.GetClrTypeFullName(type)), param));
                }
        ...
        {
            serializationExceptionMessage = GetInvalidCollectionMessage(message, SRHelper.Format(hasCollectionDataContract ? SR.InvalidCollectionDataContract : SR.InvalidCollectionType, DataContract.GetClrTypeFullName(type)), param);
            deserializationExceptionMessage = GetInvalidCollectionMessage(message, SRHelper.Format(SR.ReadOnlyCollectionDeserialization, DataContract.GetClrTypeFullName(type)), param);
        }

        private static string GetInvalidCollectionMessage(string message, string nestedMessage, string param)
        {
            return (param == null) ? SRHelper.Format(message, nestedMessage) : SRHelper.Format(message, nestedMessage, param);
        }
        ...
        private void CheckConstructor()
        {
            if (Constructor == null)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.CollectionTypeDoesNotHaveDefaultCtor, DataContract.GetClrTypeFullName(UnderlyingType))));
            }
            else
            {
