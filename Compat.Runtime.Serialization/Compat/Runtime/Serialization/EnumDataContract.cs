    ...
    internal sealed class EnumDataContract : DataContract
    {
        ...
        private class EnumDataContractCriticalHelper : DataContract.DataContractCriticalHelper
        {
                ...
                if (TryGetDCAttribute(type, out DataContractAttribute dataContractAttribute))
                {
                    if (dataContractAttribute.IsReference)
                    {
                        DataContract.ThrowInvalidDataContractException(
                                SRHelper.Format(SR.EnumTypeCannotHaveIsReference,
            ...
            internal XmlQualifiedName BaseContractName
            {
                ...
                set
                {
                    ...
                    if (baseType == null)
                    {
                        ThrowInvalidDataContractException(SRHelper.Format(SR.InvalidEnumBaseType, value.Name, value.Namespace, StableName.Name, StableName.Namespace));
                    }
                ...
                for (int i = 0; i < fields.Length; i++)
                {
                    ...
                    if (hasDataContract)
                    {
                        ...
                        if (memberAttributes != null && memberAttributes.Length > 0)
                        {
                            ...
                            {
                                ThrowInvalidDataContractException(SRHelper.Format(SR.TooManyEnumMembers, DataContract.GetClrTypeFullName(field.DeclaringType), field.Name));
                            }
                            ...
                            if (memberAttribute.IsValueSetExplicitly)
                            {
                                ...
                                {
                                    ThrowInvalidDataContractException(SRHelper.Format(SR.InvalidEnumMemberValue, field.Name, DataContract.GetClrTypeFullName(type)));
                                }
                        ...
                        {
                            ThrowInvalidDataContractException(SRHelper.Format(SR.DataMemberOnEnumField, DataContract.GetClrTypeFullName(field.DeclaringType), field.Name));
                        }
                    }
                    else
                    {
        ...
        internal void WriteEnumValue(XmlWriterDelegator writer, object value)
        {
            ...
            if (IsFlags)
            {
                ...
                if (longValue != 0)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.InvalidEnumValueOnWrite, value, DataContract.GetClrTypeFullName(UnderlyingType))));
                }
        ...
        internal object ReadEnumValue(XmlReaderDelegator reader)
        {
            ...
            else
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.InvalidEnumValueOnRead, stringValue, DataContract.GetClrTypeFullName(UnderlyingType))));
                }
        ...
        private long ReadEnumValue(string value, int index, int count)
        {
            ...
            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.InvalidEnumValueOnRead, value.Substring(index, count), DataContract.GetClrTypeFullName(UnderlyingType))));
        }
