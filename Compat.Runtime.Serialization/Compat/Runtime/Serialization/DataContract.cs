...
namespace Compat.Runtime.Serialization
{
    ...
    internal abstract class DataContract
    {
        ...
        internal static DataContract GetGetOnlyCollectionDataContract(int id, RuntimeTypeHandle typeHandle, Type type, SerializationMode mode)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new SerializationException(SRHelper.Format(SR.ClassDataContractReturnedForGetOnlyCollection, DataContract.GetClrTypeFullName(dataContract.UnderlyingType))));
            }
            return dataContract;
        }
        ...
        {
            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.UnexpectedContractType, DataContract.GetClrTypeFullName(GetType()), DataContract.GetClrTypeFullName(UnderlyingType))));
        }
                        ...
                        if (dataContract == null)
                        {
                            ...
                            if (dataContract == null)
                            {
                                ...
                                else
                                {
                                    ...
                                    //    ThrowInvalidDataContractException(SRHelper.Format(SR.TypeMustNotBeOpenGeneric, type), type);
                                    if (type.IsPointer)
                                    {
                                    ...
                                    if (!CollectionDataContract.TryCreate(type, out dataContract))
                                    {
                                        ...
                                        {
                                            ThrowInvalidDataContractException(SRHelper.Format(SR.TypeNotSerializable, type), type);
                                        }
                                    }
                                }
                            }
        ...
        private static void ValidatePreviousCollectionTypes(Type collectionType, Type itemType, Dictionary<Type, object> previousCollectionTypes)
        {
            ...
            while (itemTypeQueue.Count > 0)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.RecursiveCollectionType, DataContract.GetClrTypeFullName(itemType))));
                }
        ...
        private static XmlQualifiedName GetDCTypeStableName(Type type, DataContractAttribute dataContractAttribute)
        {
            ...
            if (dataContractAttribute.IsNameSetExplicitly)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidDataContractName, DataContract.GetClrTypeFullName(type))));
                }
            ...
            if (dataContractAttribute.IsNamespaceSetExplicitly)
            {
                ...
                if (ns == null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidDataContractNamespace, DataContract.GetClrTypeFullName(type))));
                }
        ...
        internal static bool TryGetDCAttribute(Type type, out DataContractAttribute dataContractAttribute)
        {
            ...
            if (dataContractAttributes != null && dataContractAttributes.Length > 0)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.TooManyDataContracts, DataContract.GetClrTypeFullName(type))));
                }
#endif
        ...
        private static XmlQualifiedName GetCollectionStableName(Type type, Type itemType, Dictionary<Type, object> previousCollectionTypes, out CollectionDataContractAttribute collectionContractAttribute)
        {
            ...
            if (collectionContractAttributes != null && collectionContractAttributes.Length > 0)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.TooManyCollectionContracts, DataContract.GetClrTypeFullName(type))));
                }
#endif
                ...
                if (collectionContractAttribute.IsNameSetExplicitly)
                {
                    ...
                    if (name == null || name.Length == 0)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidCollectionContractName, DataContract.GetClrTypeFullName(type))));
                    }
                ...
                if (collectionContractAttribute.IsNamespaceSetExplicitly)
                {
                    ...
                    if (ns == null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidCollectionContractNamespace, DataContract.GetClrTypeFullName(type))));
                    }
        ...
        private static void CheckExplicitDataContractNamespaceUri(string dataContractNs, Type type)
        {
            if (dataContractNs.Length > 0)
            {
                ...
                {
                    ThrowInvalidDataContractException(SRHelper.Format(SR.DataContractNamespaceIsNotValid, dataContractNs), type);
                }
            ...
            if (Uri.TryCreate(dataContractNs, UriKind.RelativeOrAbsolute, out Uri uri))
            {
                ...
                {
                    ThrowInvalidDataContractException(SRHelper.Format(SR.DataContractNamespaceReserved, Globals.SerializationNamespace), type);
                }
            }
            else
            {
        ...
        private static string GetGlobalDataContractNamespace(string clrNs, ICustomAttributeProvider customAttribuetProvider)
        {
            ...
            for (int i = 0; i < nsAttributes.Length; i++)
            {
                ...
                if (clrNsInAttribute == clrNs)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidGlobalDataContractNamespace, clrNs)));
                    }

                    if (dataContractNs != null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.DataContractNamespaceAlreadySet, dataContractNs, nsAttribute.ContractNamespace, clrNs)));
                    }
        ...
        internal static string ExpandGenericParameters(string format, IGenericNameProvider genericNameProvider)
        {
            ...
            for (int i = 0; i < format.Length; i++)
            {
                ...
                if (ch == '{')
                {
                    ...
                    if (i == format.Length)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GenericNameBraceMismatch, format, genericNameProvider.GetGenericTypeName())));
                    }
                    ...
                    else
                    {
                        ...
                        {
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GenericParameterNotValid, format.Substring(start, i - start), genericNameProvider.GetGenericTypeName(), genericNameProvider.GetParameterCount() - 1)));
                        }
                ...
                if (knownTypeAttributes != null)
                {
                    ...
                    for (int i = 0; i < knownTypeAttributes.Length; ++i)
                    {
                        ...
                        if (kt.Type != null)
                        {
                            if (useMethod)
                            {
                                DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.KnownTypeAttributeOneScheme, DataContract.GetClrTypeFullName(type)), type);
                            }
                        ...
                        else
                        {
                            ...
                            if (methodName == null)
                            {
                                DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.KnownTypeAttributeNoData, DataContract.GetClrTypeFullName(type)), type);
                            }

                            if (methodName.Length == 0)
                            {
                                DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.KnownTypeAttributeEmptyString, DataContract.GetClrTypeFullName(type)), type);
                            }
                            ...
                            if (method == null)
                            {
                                DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.KnownTypeAttributeUnknownMethod, methodName, DataContract.GetClrTypeFullName(type)), type);
                            }

                            if (!Globals.TypeOfTypeEnumerable.IsAssignableFrom(method.ReturnType))
                            {
                                DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.KnownTypeAttributeReturnType, DataContract.GetClrTypeFullName(type), methodName), type);
                            }

                            object types = method.Invoke(null, Globals.EmptyObjectArray);
                            if (types == null)
                            {
                                DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.KnownTypeAttributeMethodNull, DataContract.GetClrTypeFullName(type)), type);
                            }
                            ...
                            foreach (Type ty in (IEnumerable<Type>)types)
                            {
                                if (ty == null)
                                {
                                    DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.KnownTypeAttributeValidMethodTypes, DataContract.GetClrTypeFullName(type)), type);
                                }
        ...
        private static void CheckRootTypeInConfigIsGeneric(Type type, ref Type rootType, ref Type[] genArgs)
        {
            if (rootType.IsGenericType)
            {
                ...
                {
                    DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.TypeMustBeConcrete, type), type);
                }
            }
        }
        ...
        private static bool IsCollectionElementTypeEqualToRootType(string collectionElementTypeName, Type rootType)
        {
            if (collectionElementTypeName.StartsWith(DataContract.GetClrTypeFullName(rootType), StringComparison.Ordinal))
            {
                ...
                if (t != null)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(SRHelper.Format(SR.KnownTypeConfigClosedGenericDeclared, collectionElementTypeName)));
                    }
        ...
        internal static void CheckAndAdd(Type type, Dictionary<Type, Type> typesChecked, ref DataContractDictionary nameToDataContractTable)
        {
            ...
            else if (nameToDataContractTable.TryGetValue(dataContract.StableName, out DataContract alreadyExistingContract))
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidOperationException(SRHelper.Format(SR.DupContractInKnownTypes, type, alreadyExistingContract.UnderlyingType, dataContract.StableName.Namespace, dataContract.StableName.Name)));
                }
