        ...
        private class CriticalHelper
        {
            ...
            private LocalBuilder ReadValue(Type type, string name, string ns)
            {
                ...
                if ((primitiveContract != null && primitiveContract.UnderlyingType != Globals.TypeOfObject) || nullables != 0 || type.IsValueType)
                {
                    ...
                    {
                        ThrowValidationException(SRHelper.Format(SR.ValueTypeCannotBeNull, DataContract.GetClrTypeFullName(type)));
                    }
                    else
                    {
                        ilg.Load(null);
                    ...
                    if (type.IsValueType)
                    {
                        ...
                        ThrowValidationException(SRHelper.Format(SR.ValueTypeCannotHaveId, DataContract.GetClrTypeFullName(type)));
                        ilg.EndIf();
                    }
                    if (nullables != 0)
                    {
                    ...
                    if (type.IsValueType)
                    {
                        ThrowValidationException(SRHelper.Format(SR.ValueTypeCannotHaveRef, DataContract.GetClrTypeFullName(type)));
                    }
                    else
                    {
