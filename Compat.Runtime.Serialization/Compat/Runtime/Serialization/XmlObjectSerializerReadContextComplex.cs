    ...
    internal class XmlObjectSerializerReadContextComplex : XmlObjectSerializerReadContext
    {
        ...
        internal override DataContract GetDataContract(int id, RuntimeTypeHandle typeHandle)
        {
            ...
            if (dataContract != null)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new System.Runtime.Serialization.InvalidDataContractException(SRHelper.Format(SR.SurrogatesWithGetOnlyCollectionsNotSupportedSerDeser,
        ...
        private object InternalDeserializeInSharedTypeMode(XmlReaderDelegator xmlReader, int declaredTypeID, Type declaredType, string name, string ns)
        {
            ...
            if (assemblyName != null && typeName != null)
            {
                ...
                if (dataContract == null)
                {
                    if (assembly == null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.AssemblyNotFound, assemblyName)));
                    }

                    if (type == null)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.ClrTypeNotFound, assembly.FullName, typeName)));
                    }
                }
            ...
            else
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(XmlObjectSerializer.TryAddLineInfo(xmlReader, SRHelper.Format(SR.AttributeNotFound, Globals.SerializationNamespace, Globals.ClrTypeLocalName, xmlReader.NodeType, xmlReader.NamespaceURI, xmlReader.LocalName))));
                }
        ...
        internal override void CheckIfTypeSerializable(Type memberType, bool isMemberTypeSerializable)
        {
            ...
            else
            {
                if (dataContractSurrogate != null)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new System.Runtime.Serialization.InvalidDataContractException(SRHelper.Format(SR.TypeNotSerializable, memberType)));
                    }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private static void CheckTypeForwardedTo(Assembly sourceAssembly, Assembly destinationAssembly, Type resolvedType)
        {
            if (sourceAssembly != destinationAssembly && !NetDataContractSerializer.UnsafeTypeForwardingEnabled && !sourceAssembly.IsFullyTrusted)
            {
                ...
                //        }
                //    }
                //    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.CannotDeserializeForwardedType, DataContract.GetClrTypeFullName(resolvedType))));
                //}
            }
        }
