    ...
    public abstract class XmlObjectSerializer
    {
        ...
        internal void WriteObjectContentHandleExceptions(XmlWriterDelegator writer, object graph)
        {
            try
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.XmlWriterMustBeInElement, writer.WriteState)));
                }
        ...
        internal static string TryAddLineInfo(XmlReaderDelegator reader, string errorMessage)
        {
            ...
            {
                return string.Format(CultureInfo.InvariantCulture, "{0} {1}", SRHelper.Format(SR.ErrorInLine, reader.LineNumber, reader.LinePosition), errorMessage);
            }
        ...
        {
            return XmlObjectSerializer.CreateSerializationException(TryAddLineInfo(reader, SRHelper.Format(SR.EncounteredWithNameNamespace, errorMessage, reader.NodeType, reader.LocalName, reader.NamespaceURI)));
        }
        ...
        {
            string typeInfo = (type == null) ? string.Empty : SRHelper.Format(SR.ErrorTypeInfo, DataContract.GetClrTypeFullName(type));
            string innerExceptionMessage = innerException?.Message ?? string.Empty;
            return SRHelper.Format(errorMessage, typeInfo, innerExceptionMessage);
        }
