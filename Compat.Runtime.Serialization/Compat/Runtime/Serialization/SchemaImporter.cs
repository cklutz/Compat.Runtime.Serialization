    ...
    internal class SchemaImporter
    {
        ...
        private DataContract ImportAnonymousElement(XmlSchemaElement element, XmlQualifiedName typeQName)
        {
            if (SchemaHelper.GetSchemaType(SchemaObjects, typeQName) != null)
            {
                for (int i = 1; ; i++)
                {
                    ...
                    if (i == int.MaxValue)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.CannotComputeUniqueName, element.Name)));
                    }
                }
            }
            if (element.SchemaType == null)
        ...
        private DataContract ImportType(XmlQualifiedName typeName)
        {
            ...
            if (dataContract == null)
            {
                ...
                if (type == null)
                {
                    throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.SpecifiedTypeNotFoundInSchema, typeName.Name, typeName.Namespace)));
                }
        ...
        private ClassDataContract ImportClass(XmlQualifiedName typeName, XmlSchemaSequence rootSequence, XmlQualifiedName baseTypeName, XmlSchemaAnnotation annotation, bool isReference)
        {
            ...
            if (baseTypeName != null)
            {
                ...
                if (dataContract.BaseContract.IsISerializable)
                {
                    ...
                    {
                        ThrowTypeCannotBeImportedException(dataContract.StableName.Name, dataContract.StableName.Namespace, SRHelper.Format(SR.DerivedTypeNotISerializable, baseTypeName.Name, baseTypeName.Namespace));
                    }
                }
        ...
        private bool IsValueType(XmlQualifiedName typeName, XmlSchemaAnnotation annotation)
        {
            ...
            if (isValueTypeInnerText != null)
            {
                ...
                {
                    ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.IsValueTypeFormattedIncorrectly, isValueTypeInnerText, fe.Message));
                }
            }
            return false;
        }
        ...
        private ClassDataContract ImportISerializable(XmlQualifiedName typeName, XmlSchemaSequence rootSequence, XmlQualifiedName baseTypeName, XmlSchemaObjectCollection attributes, XmlSchemaAnnotation annotation)
        {
            ...
            else
            {
                ...
                if (!dataContract.BaseContract.IsISerializable)
                {
                    ThrowISerializableTypeCannotBeImportedException(dataContract.StableName.Name, dataContract.StableName.Namespace, SRHelper.Format(SR.BaseTypeNotISerializable, baseTypeName.Name, baseTypeName.Namespace));
                }
        ...
        private void CheckISerializableBase(XmlQualifiedName typeName, XmlSchemaSequence rootSequence, XmlSchemaObjectCollection attributes)
        {
            ...
            {
                ThrowISerializableTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ISerializableWildcardMinOccursMustBe, iSerializableWildcardElement.MinOccurs));
            }

            if (wildcard.MaxOccursString != iSerializableWildcardElement.MaxOccursString)
            {
                ThrowISerializableTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ISerializableWildcardMaxOccursMustBe, iSerializableWildcardElement.MaxOccursString));
            }

            if (wildcard.Namespace != iSerializableWildcardElement.Namespace)
            {
                ThrowISerializableTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ISerializableWildcardNamespaceInvalid, iSerializableWildcardElement.Namespace));
            }

            if (wildcard.ProcessContents != iSerializableWildcardElement.ProcessContents)
            {
                ThrowISerializableTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ISerializableWildcardProcessContentsInvalid, iSerializableWildcardElement.ProcessContents));
            }
            ...
            {
                ThrowISerializableTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ISerializableMustRefFactoryTypeAttribute, factoryTypeAttributeRefName.Name, factoryTypeAttributeRefName.Namespace));
            }
        }
        ...
        private void ImportBaseContract(XmlQualifiedName baseTypeName, ClassDataContract dataContract)
        {
            ...
            {
                ThrowTypeCannotBeImportedException(dataContract.StableName.Name, dataContract.StableName.Namespace, SRHelper.Format(dataContract.IsISerializable ? SR.InvalidISerializableDerivation : SR.InvalidClassDerivation, baseTypeName.Name, baseTypeName.Namespace));
            }
        ...
        private void ImportTopLevelElement(XmlQualifiedName typeName)
        {
            ...
            else
            {
                ...
                if (elementTypeName.IsEmpty)
                {
                    ...
                    {
                        ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.AnonymousTypeNotSupported, typeName.Name, typeName.Namespace));
                    }
                    else
                    {
                ...
                {
                    ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.TopLevelElementRepresentsDifferentType, topLevelElement.SchemaTypeName.Name, topLevelElement.SchemaTypeName.Namespace));
                }
        ...
        private void ImportClassMember(XmlSchemaElement element, ClassDataContract dataContract)
        {
            ...
            {
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ElementMinOccursMustBe, element.Name));
            }

            if (element.MaxOccurs != 1)
            {
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ElementMaxOccursMustBe, element.Name));
            }
            ...
            {
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.FormMustBeQualified, element.Name));
            }
            ...
            if (element.SchemaTypeName.IsEmpty)
            {
                ...
                {
                    ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ElementRefOnLocalElementNotSupported, element.RefName.Name, element.RefName.Namespace));
                }
                else
                {
            ...
            if (!memberTypeContract.IsValueType && !memberIsNullable)
            {
                ...
                {
                    throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.InvalidEmitDefaultAnnotation, memberName, typeName.Name, typeName.Namespace)));
                }
            ...
            if (prevMemberIndex >= 0)
            {
                ...
                if (compare == 0)
                {
                    ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.CannotHaveDuplicateElementNames, memberName));
                }
                else if (compare > 0)
                {
                    memberOrder = dataContract.Members.Count;
        ...
        private bool? ImportEmitDefaultValue(XmlSchemaAnnotation annotation, XmlQualifiedName typeName)
        {
            ...
            {
                throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.AnnotationAttributeNotFound, SchemaExporter.DefaultValueAnnotation.Name, typeName.Name, typeName.Namespace, Globals.EmitDefaultValueAttribute)));
            }
        ...
        internal static XmlQualifiedName ImportActualType(XmlSchemaAnnotation annotation, XmlQualifiedName defaultTypeName, XmlQualifiedName typeName)
        {
            ...
            if (name == null)
            {
                throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.AnnotationAttributeNotFound, SchemaExporter.ActualTypeAnnotationName.Name, typeName.Name, typeName.Namespace, Globals.ActualTypeNameAttribute)));
            }
            ...
            if (ns == null)
            {
                throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.AnnotationAttributeNotFound, SchemaExporter.ActualTypeAnnotationName.Name, typeName.Name, typeName.Namespace, Globals.ActualTypeNamespaceAttribute)));
            }
        ...
        private CollectionDataContract ImportCollection(XmlQualifiedName typeName, XmlSchemaSequence rootSequence, XmlSchemaObjectCollection attributes, XmlSchemaAnnotation annotation, bool isReference)
        {
            ...
            if (elementForm != XmlSchemaForm.Qualified)
            {
                ThrowArrayTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ArrayItemFormMustBe, element.Name));
            }
            ...
            if (element.SchemaTypeName.IsEmpty)
            {
                ...
                {
                    ThrowArrayTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.ElementRefOnLocalElementNotSupported, element.RefName.Name, element.RefName.Namespace));
                }
                else
                {
            ...
            if (IsDictionary(typeName, annotation))
            {
                ...
                {
                    ThrowArrayTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.InvalidKeyValueType, element.Name));
                }
                if (keyValueContract.Namespace != dataContract.Namespace)
                {
                    ThrowArrayTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.InvalidKeyValueTypeNamespace, element.Name, keyValueContract.Namespace));
                }
        ...
        private bool IsDictionary(XmlQualifiedName typeName, XmlSchemaAnnotation annotation)
        {
            ...
            if (isDictionaryInnerText != null)
            {
                ...
                {
                    ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.IsDictionaryFormattedIncorrectly, isDictionaryInnerText, fe.Message));
                }
            }
            return false;
        }
        ...
        private EnumDataContract ImportFlagsEnum(XmlQualifiedName typeName, XmlSchemaSimpleTypeList list, XmlSchemaAnnotation annotation)
        {
            ...
            {
                ThrowEnumTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.EnumListMustContainAnonymousType));
            }

            XmlSchemaSimpleTypeContent content = anonymousType.Content;
            if (content is XmlSchemaSimpleTypeUnion)
            {
                ThrowEnumTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.EnumUnionInAnonymousTypeNotSupported));
            }
            else if (content is XmlSchemaSimpleTypeList)
            {
                ThrowEnumTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.EnumListInAnonymousTypeNotSupported));
            }
            else if (content is XmlSchemaSimpleTypeRestriction)
            {
                XmlSchemaSimpleTypeRestriction restriction = (XmlSchemaSimpleTypeRestriction)content;
        ...
        private GenericInfo ImportGenericInfo(XmlElement typeElement, XmlSchemaType type)
        {
            ...
            if (name == null)
            {
                throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GenericAnnotationAttributeNotFound, type.Name, Globals.GenericNameAttribute)));
            }
            ...
            if (ns == null)
            {
                throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GenericAnnotationAttributeNotFound, type.Name, Globals.GenericNamespaceAttribute)));
            }
            ...
            foreach (XmlNode childNode in typeElement.ChildNodes)
            {
                ...
                {
                    throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GenericAnnotationHasInvalidElement, argumentElement.LocalName, argumentElement.NamespaceURI, type.Name)));
                }
                ...
                if (nestedLevelAttribute != null)
                {
                    ...
                    {
                        throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GenericAnnotationHasInvalidAttributeValue, argumentElement.LocalName, argumentElement.NamespaceURI, type.Name, nestedLevelAttribute.Value, nestedLevelAttribute.LocalName, Globals.TypeOfInt.Name)));
                    }
                }
                if (argumentLevel < currentLevel)
                {
                    throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GenericAnnotationForNestedLevelMustBeIncreasing, argumentElement.LocalName, argumentElement.NamespaceURI, type.Name)));
                }
            ...
            if (typeNestedLevelsAttribute != null)
            {
                ...
                {
                    throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.GenericAnnotationHasInvalidAttributeValue, typeElement.LocalName, typeElement.NamespaceURI, type.Name, typeNestedLevelsAttribute.Value, typeNestedLevelsAttribute.LocalName, Globals.TypeOfInt.Name)));
                }
        ...
        private void CheckIfElementUsesUnsupportedConstructs(XmlQualifiedName typeName, XmlSchemaElement element)
        {
            ...
            {
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.AbstractElementNotSupported, element.Name));
            }

            if (element.DefaultValue != null)
            {
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.DefaultOnElementNotSupported, element.Name));
            }

            if (element.FixedValue != null)
            {
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.FixedOnElementNotSupported, element.Name));
            }

            if (!element.SubstitutionGroup.IsEmpty)
            {
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.SubstitutionGroupOnElementNotSupported, element.Name));
            }
        }
            ...
            if (attributes != null)
            {
                ...
                for (int i = 0; i < attributes.Count; i++)
                {
                    ...
                    if (o is XmlSchemaAttribute)
                    {
                        ...
                        {
                            ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.TypeShouldNotContainAttributes, Globals.SerializationNamespace));
                        }
                    }
                }
        ...
        private bool TryCheckIfAttribute(XmlQualifiedName typeName, XmlSchemaAttribute attribute, XmlQualifiedName refName, ref bool foundAttribute)
        {
            ...
            if (foundAttribute)
            {
                ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.CannotHaveDuplicateAttributeNames, refName.Name));
            }
        ...
        private string GetInnerText(XmlQualifiedName typeName, XmlElement xmlElement)
        {
            if (xmlElement != null)
            {
                ...
                while (child != null)
                {
                    ...
                    {
                        ThrowTypeCannotBeImportedException(typeName.Name, typeName.Namespace, SRHelper.Format(SR.InvalidAnnotationExpectingText, xmlElement.LocalName, xmlElement.NamespaceURI, child.LocalName, child.NamespaceURI));
                    }
        ...
        {
            ThrowTypeCannotBeImportedException(SRHelper.Format(SR.TypeCannotBeImported, name, ns, message));
        }

        private static void ThrowArrayTypeCannotBeImportedException(string name, string ns, string message)
        {
            ThrowTypeCannotBeImportedException(SRHelper.Format(SR.ArrayTypeCannotBeImported, name, ns, message));
        }

        private static void ThrowEnumTypeCannotBeImportedException(string name, string ns, string message)
        {
            ThrowTypeCannotBeImportedException(SRHelper.Format(SR.EnumTypeCannotBeImported, name, ns, message));
        }

        private static void ThrowISerializableTypeCannotBeImportedException(string name, string ns, string message)
        {
            ThrowTypeCannotBeImportedException(SRHelper.Format(SR.ISerializableTypeCannotBeImported, name, ns, message));
        }

        private static void ThrowTypeCannotBeImportedException(string message)
        {
            throw Compat.Runtime.Serialization.DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.TypeCannotBeImportedHowToFix, message)));
        }
    }
