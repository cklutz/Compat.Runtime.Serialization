    ...
    internal sealed class DataContractSerializer : XmlObjectSerializer
    {
        ...
        internal void InternalWriteObjectContent(XmlWriterDelegator writer, object graph, DataContractResolver dataContractResolver)
        {
            ...
            if (graph == null)
            {
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.IsAnyCannotBeNull, declaredType)));
                }
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (IsRootXmlAny(rootName, contract))
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.IsAnyCannotBeSerializedAsDerivedType, graphType, contract.UnderlyingType)));
                    }
        ...
        internal override object InternalReadObject(XmlReaderDelegator xmlReader, bool verifyObjectName, DataContractResolver dataContractResolver)
        {
            ...
            if (verifyObjectName)
            {
                if (!InternalIsStartObject(xmlReader))
                {
                    ...
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationExceptionWithReaderDetails(SRHelper.Format(SR.ExpectingElement, expectedNs, expectedName), xmlReader));
                }
            }
