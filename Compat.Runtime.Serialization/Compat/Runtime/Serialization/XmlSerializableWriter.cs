...
namespace Compat.Runtime.Serialization
{
    internal class XmlSerializableWriter : XmlWriter
    {
        ...
        internal void EndWrite()
        {
            if (_depth != 0)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.IXmlSerializableMissingEndElements, (_obj == null ? string.Empty : DataContract.GetClrTypeFullName(_obj.GetType())))));
            }
        ...
        public override void WriteEndElement()
        {
            if (_depth == 0)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.IXmlSerializableWritePastSubTree, (_obj == null ? string.Empty : DataContract.GetClrTypeFullName(_obj.GetType())))));
            }
