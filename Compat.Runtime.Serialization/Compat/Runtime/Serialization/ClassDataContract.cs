        ...
        internal static void CheckAndAddMember(List<DataMember> members, DataMember memberContract, Dictionary<string, DataMember> memberNamesTable)
        {
            if (memberNamesTable.TryGetValue(memberContract.Name, out DataMember existingMemberContract))
            {
                ...
                DataContract.ThrowInvalidDataContractException(
                    SRHelper.Format((declaringType.IsEnum ? SR.DupEnumMemberValue : SR.DupMemberName),
            ...
            if (!IsTypeVisible(UnderlyingType))
            {
                if (securityException != null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(
                        new SecurityException(SRHelper.Format(
        ...
        private class ClassDataContractCriticalHelper : DataContract.DataContractCriticalHelper
        {
                ...
                : base(type)
            {
                ...
                if (isISerializable)
                {
                    if (HasDataContract)
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.ISerializableCannotHaveDataContract, DataContract.GetClrTypeFullName(type))));
                    }
                ...
                if (baseType != null && baseType != Globals.TypeOfObject && baseType != Globals.TypeOfValueType && baseType != Globals.TypeOfUri)
                {
                    ...
                    {
                        throw DiagnosticUtility.ExceptionUtility.ThrowHelperError
                            (new InvalidDataContractException(SRHelper.Format(SR.AttributedTypesCannotInheritFromNonAttributedSerializableTypes,
                ...
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.OnlyDataContractTypesCanHaveExtensionData, DataContract.GetClrTypeFullName(type))));
                }
            ...
            private void EnsureIsReferenceImported(Type type)
            {
                ...
                if (BaseContract != null)
                {
                    if (hasDataContractAttribute && dataContractAttribute.IsReferenceSetExplicitly)
                    {
                        ...
                        {
                            DataContract.ThrowInvalidDataContractException(
                                    SRHelper.Format(SR.InconsistentIsReference,
                ...
                {
                    DataContract.ThrowInvalidDataContractException(
                            SRHelper.Format(SR.ValueTypeCannotHaveIsReference,
                                DataContract.GetClrTypeFullName(type),
                                true,
                                false),
                            type);
                ...
                for (int i = 0; i < memberInfos.Length; i++)
                {
                    ...
                    if (HasDataContract)
                    {
                        ...
                        if (memberAttributes != null && memberAttributes.Length > 0)
                        {
                            ...
                            {
                                ThrowInvalidDataContractException(SRHelper.Format(SR.TooManyDataMembers, DataContract.GetClrTypeFullName(member.DeclaringType), member.Name));
                            }
                            ...
                            if (member.MemberType == MemberTypes.Property)
                            {
                                ...
                                if (getMethod == null)
                                {
                                    ThrowInvalidDataContractException(SRHelper.Format(SR.NoGetMethodForProperty, property.DeclaringType, property.Name));
                                }
                                ...
                                if (setMethod == null)
                                {
                                    ...
                                    {
                                        serializationExceptionMessage = SRHelper.Format(SR.NoSetMethodForProperty, property.DeclaringType, property.Name);
                                    }
                                }
                                if (getMethod.GetParameters().Length > 0)
                                {
                                    ThrowInvalidDataContractException(SRHelper.Format(SR.IndexedPropertyCannotBeSerialized, property.DeclaringType, property.Name));
                                }
                            }
                            else if (member.MemberType != MemberTypes.Field)
                            {
                                ThrowInvalidDataContractException(SRHelper.Format(SR.InvalidMember, DataContract.GetClrTypeFullName(type), member.Name));
                            }
                            ...
                            if (memberAttribute.IsNameSetExplicitly)
                            {
                                ...
                                {
                                    ThrowInvalidDataContractException(SRHelper.Format(SR.InvalidDataMemberName, member.Name, DataContract.GetClrTypeFullName(type)));
                                }
                            ...
                            if (memberAttribute.IsRequired && IsReference)
                            {
                                ThrowInvalidDataContractException(
                                    SRHelper.Format(SR.IsRequiredDataMemberOnIsReferenceDataContractType,
                    ...
                    else if (isNonAttributedType)
                    {
                        ...
                        if (memberAttributes != null && memberAttributes.Length > 0)
                        {
                            if (memberAttributes.Length > 1)
                            {
                                ThrowInvalidDataContractException(SRHelper.Format(SR.TooManyIgnoreDataMemberAttributes, DataContract.GetClrTypeFullName(member.DeclaringType), member.Name));
                            }
                            else
                            {
                                continue;
                    ...
                    else
                    {
                        ...
                        if (field != null && !field.IsNotSerialized)
                        {
                            ...
                            if (optionalFields == null || optionalFields.Length == 0)
                            {
                                ...
                                {
                                    ThrowInvalidDataContractException(
                                        SRHelper.Format(SR.NonOptionalFieldMemberOnIsReferenceSerializableType,
                                        DataContract.GetClrTypeFullName(member.DeclaringType),
                                        member.Name, true), type);
                                }
            ...
            private bool IsValidExtensionDataSetMethod(MethodInfo method, ParameterInfo[] parameters)
            {
                if (method.Name == Globals.ExtensionDataSetExplicitMethod || method.Name == Globals.ExtensionDataSetMethod)
                {
                    ...
                    {
                        ThrowInvalidDataContractException(SRHelper.Format(SR.DuplicateExtensionDataSetMethod, method, extensionDataSetMethod, DataContract.GetClrTypeFullName(method.DeclaringType)));
                    }

                    if (method.ReturnType != Globals.TypeOfVoid)
                    {
                        DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.ExtensionDataSetMustReturnVoid, DataContract.GetClrTypeFullName(method.DeclaringType), method), method.DeclaringType);
                    }

                    if (parameters == null || parameters.Length != 1 || parameters[0].ParameterType != Globals.TypeOfExtensionDataObject)
                    {
                        DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.ExtensionDataSetParameterInvalid, DataContract.GetClrTypeFullName(method.DeclaringType), method, Globals.TypeOfExtensionDataObject), method.DeclaringType);
                    }
            ...
            private static bool IsValidCallback(MethodInfo method, ParameterInfo[] parameters, Type attributeType, MethodInfo currentCallback, ref Type prevAttributeType)
            {
                if (method.IsDefined(attributeType, false))
                {
                    ...
                    {
                        DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.DuplicateCallback, method, currentCallback, DataContract.GetClrTypeFullName(method.DeclaringType), attributeType), method.DeclaringType);
                    }
                    else if (prevAttributeType != null)
                    {
                        DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.DuplicateAttribute, prevAttributeType, attributeType, DataContract.GetClrTypeFullName(method.DeclaringType), method), method.DeclaringType);
                    }
                    else if (method.IsVirtual)
                    {
                        DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.CallbacksCannotBeVirtualMethods, method, DataContract.GetClrTypeFullName(method.DeclaringType), attributeType), method.DeclaringType);
                    }
                    else
                    {
                        if (method.ReturnType != Globals.TypeOfVoid)
                        {
                            DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.CallbackMustReturnVoid, DataContract.GetClrTypeFullName(method.DeclaringType), method), method.DeclaringType);
                        }

                        if (parameters == null || parameters.Length != 1 || parameters[0].ParameterType != Globals.TypeOfStreamingContext)
                        {
                            DataContract.ThrowInvalidDataContractException(SRHelper.Format(SR.CallbackParameterInvalid, DataContract.GetClrTypeFullName(method.DeclaringType), method, Globals.TypeOfStreamingContext), method.DeclaringType);
                        }
            ...
            internal ClassDataContract BaseContract
            {
                ...
                set
                {
                    ...
                    {
                        ThrowInvalidDataContractException(SRHelper.Format(SR.ValueTypeCannotHaveBaseType, StableName.Name, StableName.Namespace, baseContract.StableName.Name, baseContract.StableName.Namespace));
                    }
                }
            }
            ...
            internal string DeserializationExceptionMessage
            {
                get
                {
                    ...
                    {
                        return SRHelper.Format(SR.ReadOnlyClassDeserialization, serializationExceptionMessage);
                    }
                }
            }
            ...
            internal ConstructorInfo GetISerializableConstructor()
            {
                ...
                if (ctor == null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(XmlObjectSerializer.CreateSerializationException(SRHelper.Format(SR.SerializationInfo_ConstructorNotFound, DataContract.GetClrTypeFullName(UnderlyingType))));
                }
            ...
            internal ConstructorInfo GetNonAttributedTypeConstructor()
            {
                ...
                if (ctor == null)
                {
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new InvalidDataContractException(SRHelper.Format(SR.NonAttributedSerializableTypesMustHaveDefaultConstructor, DataContract.GetClrTypeFullName(type))));
                }
